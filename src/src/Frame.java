...
public class Frame extends javax.swing.JFrame {
    public CBoard game_board = new CBoard();
    public CScore score = new CScore();
    String player; 
    boolean verify;
    RightMovement r = new RightMovement();
    UpMovement u = new UpMovement();
    LeftMovement l = new LeftMovement();
    DownMovement d = new DownMovement();
    
    public void Game_Lost(){
        //player perdeu o jogo
        score.check_record(player, score.movements);
        jTextField32.setText(score.player_record);
        jTextField33.setText(Integer.toString(score.score_record));
        jTextField34.setText("Você conseguiu "+score.movements+ " movimentos!");
        New_Game();
    }
    
    public void New_Game(){
        game_board = new CBoard();
        score.movements = 0;
        ...
        game_board.matrix[linha][coluna] = new CBlock(1);
        jTextField27.setText(Integer.toString(score.movements));
        check(game_board);
    }
    ...
    public void Movement_Done() {
        try{
            if(verify == false){
                ...
                while(true){
                    if(game_board.matrix[line][column].value != 0){
                        line = random.nextInt(5);
                        column = random.nextInt(5);
                    }
                    else if(game_board.matrix[line][column].value == 0){
                        game_board.matrix[line][column] = new CBlock(1);
                        break;
                    }
                }
                line = random.nextInt(5);
                    ...
                    while(true){
                        if(game_board.matrix[line][column].value != 0){
                            ...
                            if(sorte == 0){//Não deu sorte
                                game_board.matrix[line][column] = new CBlock(2);
                            }
                            else{
                                game_board.matrix[line][column] = new CBlock(1);
                            }
    ...
    private void up_arrowMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_up_arrowMouseClicked
        verify = false;// assume que nenhum bloco se movimentou
        verify = u.goUp(game_board, verify);
        Movement_Done();
        check(game_board);
    ...
    private void left_arrowMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_left_arrowMouseClicked
        verify = false;// assume que nenhum bloco se movimentou
        verify = l.goLeft(game_board, verify);
        Movement_Done();
        check(game_board);
    ...
    private void down_arrowMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_down_arrowMouseClicked
        verify = false;// assume que nenhum bloco se movimentou
        verify = d.goDown(game_board, verify);
        Movement_Done();
        check(game_board);
